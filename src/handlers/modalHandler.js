...
module.exports = async (client, interaction) => {
    ...
    /* Handle Link Account Modal */
    if (interaction.customId === 'link_account_modal') {
        try {
            const code = interaction.fields.getTextInputValue('link_code_input').toUpperCase().trim();
            ...
            if (!codeData) {
                await interaction.reply({
                    content: '❌ Código inválido o expirado. Escribe `!link` en Rust para obtener un nuevo código.',
                    ephemeral: true
                });
                return;
            }
            ...
            if (now - codeData.createdAt > 10 * 60 * 1000) {
                ...
                await interaction.reply({
                    content: '❌ El código expiró. Escribe `!link` en Rust para obtener un nuevo código.',
                    ephemeral: true
                });
                return;
            }
            ...
            await interaction.reply({
                content: `✅ ¡Vinculación completada!\n\n🎮 Steam ID: \`${steamId}\`\n📛 Discord: **${discordTag}**\n\nYa puedes usar comandos de voz!`,
                ephemeral: true
            });

            client.log(client.intlGet(null, 'infoCap'), `${discordTag} linked to Steam ID: ${steamId}`);
            ...
            if (instance.linkLogChannelId) {
                ...
                if (logChannel && logChannel.isTextBased()) {
                    ...
                    logChannel.send({ embeds: [logEmbed] }).catch(err => {
                        console.error('Error sending link log:', err);
                    });
                }
            }
            ...
            console.error('Error in link account modal:', error);
            await interaction.reply({
                content: '❌ Error al procesar el código. Intenta de nuevo.',
                ephemeral: true
            });
            return;
        }
