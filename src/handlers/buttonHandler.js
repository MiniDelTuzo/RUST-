...
module.exports = async (client, interaction) => {
    ...
    /* Handle Link Account Button */
    if (interaction.customId === 'link_account_button') {
        const modal = new Discord.ModalBuilder()
            .setCustomId('link_account_modal')
            .setTitle('🔗 Vincula tu Cuenta');

        const codeInput = new Discord.TextInputBuilder()
            .setCustomId('link_code_input')
            .setLabel('Código de Vinculación')
            .setPlaceholder('Pega el código que recibiste con !link en Rust')
            .setStyle(Discord.TextInputStyle.Short)
            .setRequired(true)
            .setMinLength(6)
            .setMaxLength(6);

        const actionRow = new Discord.ActionRowBuilder().addComponents(codeInput);
    ...
    /* Handle Unlink Account Button */
    if (interaction.customId === 'unlink_account_button') {
        ...
        if (!linkedSteamId) {
            await interaction.reply({
                content: '❌ No estás vinculado a ninguna cuenta.',
                ephemeral: true
            });
            return;
        }
        ...
        // Desvincularse
        delete instance.steamIdToDiscord[linkedSteamId];
        client.setInstance(interaction.guildId, instance);

        await interaction.reply({
            content: `✅ Desvinculación completada.\n\n🎮 Steam ID desviculado: \`${linkedSteamId}\``,
            ephemeral: true
        });

        client.log(client.intlGet(null, 'infoCap'), `${interaction.user.tag} unlinked from Steam ID: ${linkedSteamId}`);

        // Enviar log al canal configurado
        if (instance.linkLogChannelId) {
            const logChannel = client.channels.cache.get(instance.linkLogChannelId);
            if (logChannel && logChannel.isTextBased()) {
                const logEmbed = new Discord.EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: '🎮 Steam ID', value: `\`${linkedSteamId}\``, inline: true },
