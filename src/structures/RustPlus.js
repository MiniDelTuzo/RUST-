...
const RustPlusLite = require('../structures/RustPlusLite.js');
...
class RustPlus extends RustPlusLib {
    ...
    getCommandLeaderboardPlaytime() {
        ...
        if (!bmInstance || !bmInstance.lastUpdateSuccessful) {
            return Client.client.intlGet(this.guildId, 'noData');
        }
        ...
        for (const [playerId, playerData] of Object.entries(bmInstance.players)) {
            ...
            for (let i = 0; i < playerData['connectionLog'].length; i++) {
                ...
                if (currentEntry.data.type === 0) {
                    lastLoginDate = new Date(currentEntry.data.time);

                    if (nextEntry && nextEntry.data.type === 1) {
        ...
        let result = `${Client.client.intlGet(this.guildId, 'leaderboardPlaytime')}: `;
        ...
        for (let i = 0; i < top5.length; i++) {
            ...
            result += `${i + 1}. ${player.name} (${hours}h ${minutes}m)`;
            if (i < top5.length - 1) result += ', ';
        }

        return result + '.';
    }

    getCommandLeaderboardLogins() {
        const instance = Client.client.getInstance(this.guildId);
        const server = instance.serverList[this.serverId];
        if (!server || !server.battlemetricsId) {
            return Client.client.intlGet(this.guildId, 'invalidBattlemetricsId');
        ...
        let result = `${Client.client.intlGet(this.guildId, 'leaderboardLogins')}: `;
        const top5 = playerStats.slice(0, 5);

        for (let i = 0; i < top5.length; i++) {
            const player = top5[i];
            result += `${i + 1}. ${player.name} (${player.logins})`;
            if (i < top5.length - 1) result += ', ';
        }
    ...
    getCommandLeaderboardOldest() {
        ...
        let result = `${Client.client.intlGet(this.guildId, 'leaderboardOldest')}: `;
        const top5 = playerStats.slice(0, 5);

        for (let i = 0; i < top5.length; i++) {
            const player = top5[i];
            const daysAgo = Math.floor((Date.now() - player.firstSeen) / (1000 * 60 * 60 * 24));
            result += `${i + 1}. ${player.name} (${daysAgo}d)`;
            if (i < top5.length - 1) result += ', ';
        }
